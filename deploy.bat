...
for /f "tokens=1" %%i in ('docker ps -a ^| findstr "api-rest-cliente-transbank"') do set CONTAINER_ID=%%i
if not "%CONTAINER_ID%"=="" docker rm -f %CONTAINER_ID%

REM Obtiene el ID de la imagen para eliminarl
for /f "tokens=3" %%i in ('docker images ^| findstr "api-rest-cliente-transbank"') do set IMAGE_ID=%%i
